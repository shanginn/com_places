...
//  (function($){
  (function(){
    ...
    var placesCollection = new ymaps.GeoObjectCollection({}, {geodesic: true});
    var $map = $(mapid);

    $.each(places, function(index, place) {
      var phone = place.phone.split(')');
      var fax = place.fax.split(')');
      var placeMark = new ymaps.Placemark([place.lat, place.lng], {
        //balloonContentHeader: "Балун метки",
        balloonContentBody: ''+
          '<div class="balloon-box">'+
            '<div class="align-right">'+
              '<p class="contact-address" style="margin-bottom: 2.5em;">Адрес:</p>'+
              (place.phone ? '<p>Телефон:</p>' : '') +
              (place.fax   ? '<p>Факс:</p>' : '') +
              (place.email ? '<p>Email:</p>' : '') +
            '</div>'+
            '<div class="align-left">'+
              '<p>' + place.address + '</p>'+
              (place.phone ? 
              ('<p class="phone">' + phone[0] + ')' +
                '<span class="is-bold">' + phone[1] + '</span>'+
              '</p>') : '') +
              (place.fax ? ('<p class="phone">' + fax[0] + ')' +
                '<span class="is-bold">' + fax[1] + '</span>'+
              '</p>') : '') +
      ...
      // TODO: Remove hack. Add empty sets to skipped indexes;
      while(placesCollection.toArray().length != place.id &&
        placesCollection.add(new ymaps.GeoObjectCollection()));
      placesCollection.add(placeMark, place.id);
    });
    
    myMap.geoObjects.add(placesCollection);
    myMap.setBounds(placesCollection.getBounds());
    
    $('.place-look-wrap').click(function(e) {
      var placeId = $(this).data('placeId'),
          point   = placesCollection.get(placeId);
